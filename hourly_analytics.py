...
from config import DB_CONFIG  # MySQL config for Facts
from reports_3forms import TARGET_STAGES
import daily_plans
...
def get_hourly_report_data(date_str: str) -> Dict[str, Any]:
    print(f"DEBUG: Starting get_hourly_report_data for {date_str}")
    ...
    valid_teams = set(team_to_region.keys()) # [FIX] Allowlist from DB Mappings
    ...
    for item in raw_excel_data['data']:
        ...
        # [FIX] FILTER: Only allow entities present in the DB Mapping File
        if final_key not in valid_teams:
            continue
        ...
        for group, metrics in products.items():
            ...
            if key in ['per','obliv','vaf','vetosh','ruk','stretch','bugs','resale']:
                ...
                # Special Case: China
                # 'china' key -> Quantity
                # 'china_sum' key -> Revenue
                qty = metrics.get('quantity', 0)
                
                # Setup Keys
                region_cat_totals[final_region]['china'] = region_cat_totals[final_region].get('china', 0) + qty
                region_cat_totals[final_region]['china_sum'] = region_cat_totals[final_region].get('china_sum', 0) + rev
                
                team_cat_plans[(final_region, final_key)]['china'] = team_cat_plans[(final_region, final_key)].get('china', 0) + qty
                team_cat_plans[(final_region, final_key)]['china_sum'] = team_cat_plans[(final_region, final_key)].get('china_sum', 0) + rev
    ...
    for h in hours:
        ...
        for row in raw_facts:
            if row['won_time'] and row['won_time'] <= cutoff_dt:
                ...
                # [FIX] FILTER: Only allow teams present in the DB Mapping File
                if team_name not in valid_teams:
                    continue
                    
                if team_name not in current_facts: current_facts[team_name] = {}
                if cat_key not in current_facts[team_name]: 
                    current_facts[team_name][cat_key] = {'rev': 0, 'gp': 0, 'qty': 0}
                
                rev_val = float(row['revenue'] or 0)
                gp_val = float(row['gp'] or 0)
                qty_val = float(row['count'] or 0)

                current_facts[team_name][cat_key]['rev'] += rev_val
                current_facts[team_name][cat_key]['gp'] += gp_val
                current_facts[team_name][cat_key]['qty'] += qty_val
                
                # [FIX] Special Handling for China Aggregation keys
                if cat_key == 'china':
                     # We need 'china_sum' key for Revenue Fact
                     if 'china_sum' not in current_facts[team_name]:
                         current_facts[team_name]['china_sum'] = {'rev': 0, 'gp': 0, 'qty': 0}
                     
                     current_facts[team_name]['china_sum']['rev'] += rev_val
                     current_facts[team_name]['china_sum']['gp'] += gp_val
                     # 'china' key itself keeps Quantity as its primary metric (handled in display loop)
        ...
        for entity in sorted(all_entities):
            ...
            for cat_key in all_cat_keys:
                ...
                # Fetch Region Cumulative Plan for this Category/Hour
                reg_hourly_plan_rev = 0
                reg_hourly_plan_gp = 0
                ...
                if region in region_daily_data['hourly_breakdown']:
                    ...
                    src_rev_key = cat_key
                    src_gp_key = f"{cat_key}_gp"
                    if cat_key == 'china':
                        src_gp_key = 'china_sum_gp'
                        
                    if hour_str in rb:
                        reg_hourly_plan_rev = rb[hour_str].get(src_rev_key, 0)
                        reg_hourly_plan_gp = rb[hour_str].get(src_gp_key, 0)
                    else:
                        # Fallback: if hour > max available, use max
                        if rb:
                            max_h_str = max(rb.keys())
                            if h > int(max_h_str.split(':')[0]):
                                reg_hourly_plan_rev = rb[max_h_str].get(src_rev_key, 0)
                                reg_hourly_plan_gp = rb[max_h_str].get(src_gp_key, 0)

                plan_rev = reg_hourly_plan_rev * weight
                plan_gp = reg_hourly_plan_gp * weight
                
                row[f'{cat_key}_plan_revenue'] = int(plan_rev)
                row[f'{cat_key}_plan_gp'] = int(plan_gp) 
                
                # Fact
                f_data = current_facts.get(entity, {}).get(cat_key, {})
                
                # [FIX] China Pcs Fact should use Quantity
                if cat_key == 'china':
                    row[f'{cat_key}_fact_revenue'] = f_data.get('qty', 0)
                else:

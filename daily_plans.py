...
def get_actual_sales_ytd(date_obj: datetime):
    ...
    # [FIX] Reverting to strict "Catch Up" logic:
    # Do NOT include 'Closed Lost performance'. Only 'Closed Won' etc count as success.
    stages_str = ",".join([f"'{s}'" for s in TARGET_STAGES])
    ...
    SELECT 
        ...
        productsale.amount - (productsale.count * IF(product.price_in1 > 0, product.price_in1, productsale.amount / NULLIF(productsale.count, 0) * 0.8)) as gp
    FROM opportunities
...
def calculate_daily_target(total_monthly_plan, total_fact_ytd, remaining_days):
    ...
    # [FIX] Do NOT zero out logic requested by user. 
    # If overachieved, gap is negative -> daily plan is negative (surplus).
    # This maintains the "Catch Up logic" correctly.
    ...
    for raw_region, p_data in monthly_data.items():
        ...
        for cat in calc_cats:
            ...
            if cat == 'china':
                ...
                result['daily_totals'][region]['china_sum_gp'] = d_gp # Add GP
            else:
                result['daily_totals'][region][cat] = d_rev 
                result['daily_totals'][region][f"{cat}_gp"] = d_gp # Add GP 
